package com.application.social_share_pro
...
class SocialSharePlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: MethodChannel.Result) {
        ...
        when (call.method) {
            "shareInstagramStory", "shareFacebookStory" -> handleShareStory(call, result)
            "shareOptions" -> handleShareOptions(call, result)
            "copyToClipboard" -> handleCopyToClipboard(call, result)
            "shareWhatsapp" -> handleShareWhatsapp(call, result)
            "shareSms" -> handleShareSms(call, result)
            "shareTwitter" -> handleShareTwitter(call, result)
            "shareTelegram" -> handleShareTelegram(call, result)
            "checkInstalledApps" -> checkInstalledApps(result)
            else -> result.notImplemented()
        }
    }

    private fun handleShareStory(call: MethodCall, result: MethodChannel.Result) {
        val (destination, appName, intentString) = when (call.method) {
            "shareInstagramStory" -> Triple("com.instagram.sharedSticker", "com.instagram.android", "com.instagram.share.ADD_TO_STORY")
            "shareFacebookStory" -> Triple("com.facebook.sharedSticker", "com.facebook.katana", "com.facebook.stories.ADD_TO_STORY")
            else -> return result.success("error")
        }
        ...
        val stickerImageUri = FileProvider.getUriForFile(activeContext!!, "${activeContext!!.packageName}.com.application.social_share_pro", stickerFile)

        val intent = Intent(intentString).apply {
            type = "image/*"
            addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            putExtra("interactive_asset_uri", stickerImageUri)
            putExtra("source_application", appId)
            putExtra("content_url", attributionURL)
            putExtra("top_background_color", backgroundTopColor)
        ...
        backgroundImage?.let {
            ...
            val backgroundImageUri = FileProvider.getUriForFile(activeContext!!, "${activeContext!!.packageName}.com.application.social_share_pro", backFile)
            intent.setDataAndType(backgroundImageUri, "image/*")
        }

        backgroundVideo?.let {
            val backFile = File(activeContext!!.cacheDir, backgroundVideo)
            val backgroundVideoUri = FileProvider.getUriForFile(activeContext!!, "${activeContext!!.packageName}.com.application.social_share_pro", backFile)
            intent.setDataAndType(backgroundVideoUri, "video/*")
        }

        activity?.grantUriPermission(appName, stickerImageUri, Intent.FLAG_GRANT_READ_URI_PERMISSION)
    ...
    private fun handleShareOptions(call: MethodCall, result: MethodChannel.Result) {
        ...
        val intent = Intent(Intent.ACTION_SEND).apply {
            if (image != null) {
                ...
                val imageUri = FileProvider.getUriForFile(activeContext!!, "${activeContext!!.packageName}.com.application.social_share_pro", imageFile)
                type = "image/*"
    ...
    private fun handleCopyToClipboard(call: MethodCall, result: MethodChannel.Result) {
        ...
        if (image != null) {
            val values = ContentValues(2).apply {
                ...
                put(MediaStore.Images.Media.DATA, image)
            }
            val imageUri = activeContext!!.contentResolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values)
            val clip = ClipData.newUri(activeContext!!.contentResolver, "Image", imageUri)
    ...
    private fun handleShareWhatsapp(call: MethodCall, result: MethodChannel.Result) {
        val content: String? = call.argument("content")
    ...
    private fun handleShareSms(call: MethodCall, result: MethodChannel.Result) {
    ...
    private fun handleShareTwitter(call: MethodCall, result: MethodChannel.Result) {
    ...
    private fun handleShareTelegram(call: MethodCall, result: MethodChannel.Result) {
        val content: String? = call.argument("content")
    ...
    private fun checkInstalledApps(result: MethodChannel.Result) {
        ...
        val resolvedActivities: List<ResolveInfo> = pm.queryIntentActivities(smsIntent, 0)
        apps["sms"] = resolvedActivities.isNotEmpty()
        apps["instagram"] = packages.any { it.packageName == "com.instagram.android" }
        apps["facebook"] = packages.any { it.packageName == "com.facebook.katana" }
        apps["twitter"] = packages.any { it.packageName == "com.twitter.android" }
        apps["whatsapp"] = packages.any { it.packageName == "com.whatsapp" }
        apps["telegram"] = packages.any { it.packageName == "org.telegram.messenger" }
...
//             val stickerImageFile = FileProvider.getUriForFile(activeContext!!, activeContext!!.applicationContext.packageName + ".com.application.social_share_pro", file)
//             val appId: String? = call.argument("appId")
...
//                 val backfile =  File(activeContext!!.cacheDir,backgroundImage)
//                 val backgroundImageFile = FileProvider.getUriForFile(activeContext!!, activeContext!!.applicationContext.packageName + ".com.application.social_share_pro", backfile)
...
//                 val backfile =  File(activeContext!!.cacheDir,backgroundVideo)
//                 val backgroundVideoFile = FileProvider.getUriForFile(activeContext!!, activeContext!!.applicationContext.packageName + ".com.application.social_share_pro", backfile)
...
//                 val imagefile =  File(activeContext!!.cacheDir,image)
//                 val imageFileUri = FileProvider.getUriForFile(activeContext!!, activeContext!!.applicationContext.packageName + ".com.application.social_share_pro", imagefile)
//                 intent.type = "image/*"
